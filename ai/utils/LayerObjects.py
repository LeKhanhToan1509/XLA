...
class RBFLayer(object):
    def __init__(self, weight, n_classes=10):        
        """
        weight: Initial RBF weights (typically 10×84 for MNIST digits)
        n_classes: Total number of classes (10 for MNIST, more for custom datasets)
        """
        self.n_classes = n_classes
        # Nếu n_classes > 10, mở rộng weights bằng random initialization
        if n_classes > weight.shape[0]:
            extra_classes = n_classes - weight.shape[0]
            extra_weights = np.random.normal(0, 0.1, (extra_classes, weight.shape[1]))
            self.weight = np.vstack([weight, extra_weights])
            print(f"RBF Layer: Extended from {weight.shape[0]} to {n_classes} classes")
        else:
            self.weight = weight[:n_classes]  # Chỉ lấy n_classes đầu tiên nếu cần
    ...
    def foward_prop(self, input_array, label, mode): 
        ...
        if mode == 'train':
            ...
            return np.sum(np.squeeze(loss))
        if mode == 'test':
            # (n_m,1,84) - n_m*[(n_classes,84)] = (n_m,n_classes,84)
            subtract_weight = (input_array[:,np.newaxis,:] - np.array([self.weight]*input_array.shape[0])) # (n_m,n_classes,84)
            rbf_class = np.sum(np.power(subtract_weight,2), axis=2) # (n_m, n_classes)
            class_pred = np.argmin(rbf_class, axis=1) # (n_m,)
            error01 = np.sum(label != class_pred)
            return error01, class_pred
...
class LeNet5(object):
    def __init__(self, n_classes=10, C3_mapping=None):
        """
        n_classes: Số lượng classes (10 cho MNIST, 13 cho MNIST+shapes, etc.)
        C3_mapping: Mapping configuration for C3 layer
        """
        # Default C3 mapping nếu không được truyền vào
        if C3_mapping is None:
        ...
        # Initialize RBF layer with bitmap patterns
        self.n_classes = n_classes
        bitmap = rbf_init_weight(n_classes)  # Generate bitmap for all classes
        self.RBF = RBFLayer(bitmap, n_classes)
        
        # Store n_classes for later use
        self.Output = self.RBF
